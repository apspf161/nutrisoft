...
$(document).ready(function() {
	...
	if($("#content-table-usuario").length > 0) 
	{
		dataTableUsuario = $('#content-table-usuario').DataTable(
		{
			"dom": 'ti',
			"language": {
		...
		$('#content-table-usuario tbody').on( 'click', 'tr', function () {
	        if ( $(this).hasClass('selected') ) {
	            ...
	            idSelecionado = dataTableUsuario.row( this ).data()[0];
	            $("#idUsuario").val(idSelecionado);
	        }
	    });
		
	    $('#content-table-usuario tbody').on( 'click', 'tr td.details-control', function () {
	        var tr = $(this).closest('tr');
	        var row = dataTableUsuario.row( tr );
	    ...
	    dataTableUsuario.on( 'draw', function () {
...
function carregaBotoes()
{
	$('a#btnAlterar').on('click', function(){
		...
		{
		    $(this).attr("href", "usuario/alterarUsuario/"+idSelecionado);
		}
	});
	...
	$('a#btnExcluir').on('click', function(){
		...
		else
		{
			$("[data-toggle=confirmation-excluir]").confirmation({
				...
				href:"usuario/excluirUsuario/"+idSelecionado
			});		
		}
	});
